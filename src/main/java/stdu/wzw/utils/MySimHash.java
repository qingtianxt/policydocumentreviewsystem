...
public class MySimHash {
    ...
    private BigInteger simHash() {
        ...
        List<Term> termList = StandardTokenizer.segment(this.tokens); // 分词
        ...
        stopNatures.put("w", ""); //
        int overCount = 5; //设定超频词汇的界限，当词频超过5，算作超频词 ;
        ...
        for (Term term : termList) {
            ...
            String nature = term.nature.toString(); // 分词词性;

            //1.统计去除停用词后的词频
            // 过滤停用词性，统计词频应该放在去除停用词后面
    ...
    private BigInteger hash(String source) {
        if (source == null || source.length() == 0) {
            ...
            /**
             * 当sourece 的长度过短，会导致hash算法失效，因此需要对过短的词补偿，补偿原则是在后面追加第一个字符。
             */
